...
* </tt>source_filenames</tt> is a tuple with the source files to be compressed, relative to COMPRESS_SOURCE. The files are concatenated in the order it is specified in the tuple. This option is required.
...
Note that all filenames are specified relative to COMPRESS_ROOT, and thus the source files needs to be in your COMPRESS_ROOT.
...
* </tt>COMPRESS_ROOT</tt>: This is the path where the compressed files are stored after compression. It defaults to MEDIA_ROOT, but modern Django installs will want to set this to STATIC_ROOT.
* </tt>COMPRESS_SOURCE</tt>: This is the path where source files for compression are expected to be found. It defaults to MEDIA_ROOT, but modern Django installs will want to set this to STATIC_ROOT.
* </tt>COMPRESS_URL</tt>: This is the URL that the COMPRESS_ROOT directory is available at. If this is a fully-qualified path (i.e., starts </tt>http://</tt>) then when requests are made to Django over HTTPS, then the protocol in the URL is altered to </tt>https</tt>. Defaults to MEDIA_URL, but most modern Django installs will want to set this to STATIC_URL.
...
h4. Prefix - An Alternative to COMPRESS_URL

In cases where you want to deploy your compiled script and styles to somewhere other than your COMPRESS_URL, say a Content Delivery
